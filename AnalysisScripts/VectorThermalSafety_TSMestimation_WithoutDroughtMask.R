...
aealbo = subset(aealbo, Latitude>=33 & Latitude<=45)
...
df1 = as.data.frame(matrix(nrow = nrow(aealbo), ncol = 34))
...
for (i in 1:nrow(aealbo)){
 lat = aealbo$Latitude[i]
  lon = aealbo$Longitude[i]
  ...
  colnames(ModeledTemps)[c(5,30:36)] = c("Mosq_Temp_Point", "Mosq_Temp_Lower", "Mosq_Temp_Upper", 
